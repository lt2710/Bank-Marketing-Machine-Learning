...
```{r}
pamk_mod<-class_pamk_5
pamk_dta<-cbind(dta_km,
                cluster=pamk_mod$pamobject$clustering)
medoid <- cbind(dta_km,
                row = seq(1,nrow(pamk_dta),1))%>%filter(row%in%pamk_mod$pamobject$id.med)
```
...
```{r}
predictions_medoid_glm <- logistic_glm %>%
  predict(new_data = medoid)%>%
  bind_cols(medoid %>% select(income))
```
